...
DEPTH = 3  # 搜索深度，只能是单数。如果是负数，评估函数评估的的是自己多少步之后的自己得分的最大值，
            # 并不意味着是最好的棋， 评估函数的问题
    ...
    for offset in range(-5, 1):
        ...
        for (score, shape) in shape_score:
            if tmp_shape5 == shape or tmp_shape6 == shape:
                if tmp_shape5 == (1,1,1,1,1):
                    print('wwwwwwwwwwwwwwwwwwwwwwwwwww')
                if score > max_score_shape[0]:
                    max_score_shape = (score,
                                       ((m + (0+offset) * x_decrict, n + (0+offset) * y_derice),
...
#  绘制棋盘窗口
def gobangwin():
    win = GraphWin("基于博弈树的AI五子棋", GRID_WIDTH * COLUMN, GRID_WIDTH * ROW)
...
def main():
    ...
    change = 0  # 标记当前操作属于玩家orAI，奇数为AI，偶数为玩家
    over = 0    # 标记棋局是否结束，即是否出现某一方的胜利，或棋局已满
    ...
    while over == 0:
        ...
        if change % 2 == 1:
            ...
            # 判断鼠标点击的点是否可操作
            if not ((a2, b2) in list3):
                list2.append((a2, b2))  # 加入玩家棋局
                list3.append((a2, b2))  # 加入全局棋局
